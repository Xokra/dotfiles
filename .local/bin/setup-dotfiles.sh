...
install_tpm_plugins() {
    ...
    if [ -d "$tmp_dir" ] && [ -f "$HOME/.tmux.conf" ]; then
...
if command -v tmux &> /dev/null && [ -n "$PS1" ] && [[ ! "$TERM" =~ screen ]] && [[ ! "$TERM" =~ tmux ]] && [ -z "$TMUX" ]; then
    # Check if TPM plugins need installing (only run once)
    if [ -d "$HOME/.tmux/plugins/tpm" ] && [ -f "$HOME/.tmux.conf" ] && [ ! -f "$HOME/.tmux/plugins/.tpm_installed" ]; then
        # First time - install plugins
        tmux new-session -d -s main
        tmux send-keys -t main '$HOME/.tmux/plugins/tpm/scripts/install_plugins.sh && touch $HOME/.tmux/plugins/.tpm_installed' C-m
